...
class Game:
    ...
    def auto_move(self):
        the_method = 'ucs'
        strategy, time_solver = get_move(self.level.structure[:-1], self.level.position_player, the_method)
        with open("assets/sokobanSolver/Solverlevel_" + str(the_method) +".txt", 'a') as solver_file:
            solver_file.write('%s: %s %.4f\n' % (self.index_level, len(strategy), time_solver))
